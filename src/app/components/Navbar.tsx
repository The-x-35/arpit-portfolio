...
const Navbar: React.FC = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 flex justify-between items-center p-4 transition-colors duration-300 ${isScrolled ? 'bg-black bg-opacity-100' : 'bg-transparent'} text-white`}>
      <Link 
        className="font-bold cursor-pointer text-sm sm:text-lg md:text-xl lg:text-2xl" 
        to="first" 
        smooth={true} 
        duration={500}
      >
      ...
      {isOpen && (
        <div className="fixed top-0 left-0 w-full h-screen bg-black bg-opacity-50 flex flex-col items-center justify-center space-y-8">
